...
st.set_page_config(
    page_title="Fashion Recommendation System Using Deep Learning",
    layout="wide"
)
...
<style>
    ...
    .stApp {
        background: linear-gradient(180deg, #FFFFFF 0%, #D69CB6 100%);
    }
    ...
    .stButton>button {
        background: linear-gradient(90deg, #8A537C, #D69CB6);
        color: white;
        font-weight: bold;
        border-radius: 20px;
        padding: 0.5rem 2rem;
                ...
                for idx, (_, rec) in enumerate(recommendations.iterrows()):
                    ...
                    with col:
                        try:
                            ...
                            st.markdown(f"💗 **Similarity:** `{rec['similarity_score']:.1%}`")
...
def upload_image_mode(recommender, extractor, metadata):
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            with st.spinner("✨ Analyzing your fashion taste..."):
            ...
            for idx, (_, rec) in enumerate(recommendations.iterrows()):
                ...
                with col:
                    try:
                        ...
                        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
                        st.image(rec_img, use_container_width=True)
                        st.markdown(f"✨ **Match:** `{rec['similarity_score']:.1%}`")
                        st.progress(float(rec['similarity_score']))
...
def main():
    st.markdown('<h1 style="text-align: center;">Fashion Recommendation System using Deep Learning</h1>', unsafe_allow_html=True)
    st.markdown("""
    <div class="glass-card" style="text-align:center">
        <h2> Discover Your Perfect Style</h2>
        <p>AI-powered fashion recommendations based on visual similarity</p>
    ...
    st.sidebar.markdown("""
    1. Image processed by **ResNet CNN**
    2. 512-D visual features extracted
    3. **Cosine similarity** computed
    4. Top visually similar items recommended
    """)
    ...
    <div style="text-align:center; color:gray;">
        Made using Streamlit & PyTorch<br>
